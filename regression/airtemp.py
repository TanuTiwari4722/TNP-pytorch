...
from data.airtemp import AirTemperatureDataset
...
def main():
    ...
    parser.add_argument('--num_epochs', type=int, default=500) # changed from 200 to 500 epochs. 2 trials.
    ...
    if args.expid is not None:
        args.root = osp.join(results_path, 'air', args.model, args.expid)
    else:
        args.root = osp.join(results_path, 'air', args.model)


    model_cls = getattr(load_module(f'models/{args.model}.py'), args.model.upper())
    with open(f'configs/air/{args.model}.yaml', 'r') as f:
...
def train(args, model):
    ...
    train_ds = AirTemperatureDataset(train=True)
...
def gen_evalset(args):
    ...
    eval_ds = AirTemperatureDataset(train=False) # Applied.
    ...
    path = osp.join(evalsets_path, 'air')
...
def eval_all_metrics(args, model):
    ...
    if args.t_noise is not None:
        filename += f'_{args.t_noise}'
    filename += '.log'
    logger = get_logger(osp.join(results_path, 'air', args.model, args.expid, filename), mode='w')
    logger.info(line)
...
def plot(args, model):
    ...
    # eval_ds = EMNIST(train=False, class_range=args.class_range)
    eval_ds = AirTemperatureDataset(train=False)
